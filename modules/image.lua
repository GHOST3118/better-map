...
function image:load(id)
	...
	require("map:png/png_writer").write_image_to_buffer(self, buffer)

	if not id then
		if not self.textureId then
			self.textureId = "map:runtime_"..textureId
...
function image:to_png(path)
	require("map:png/png_writer").write_image_to_file(self, path)
end

function image.from_png(path)
	return require("map:png/png_reader").read_from_file_to_image(path)
end
...
function image.from_png_buffer(buffer)
	return require("map:png/png_reader").read_from_buffer_to_image(buffer)
end
